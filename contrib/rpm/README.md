...
The RPM spec file provided here is for Whiff-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://whiff.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Whiff git repository](https://github.com/WhiffProject/Whiffcoin) but are not part of
...
Currently only `Source10` is used, for the example `whiff.conf` file.
...
in the whiff git repository. At present only an SVG version of the Whiff
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the whiff
...
needed when 0.12.1 is released, a proper fix is already in the Whiff git
master branch.
...
idea to build Whiff Core against a static version of that library compiled
...
The `whiff.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `whiff-server` package. At this point, most RPM based
...
The `whiff-server` package should have SELinux support. How to properly do
...
Whiff-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/whiff/). He has been
packaging Whiff for Fedora at least since 2012.
...
4. I renamed the `whiff` package that contains the Qt GUI to `whiff-core` as
that appears to be how the general population refers to it, in contrast to
`whiff-xt` or `whiff-classic`. I wanted to make sure the general population
