...
###   whiffcore.org and whiff.org and compares them.
...
WORKINGDIR="/tmp/whiff_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://whiff.org"
HOST2="https://whiff.org"
BASEDIR="/bin/"
VERSIONPREFIX="whiff-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'whiff-',
...
if [ $? -ne 0 ]; then
   echo "whiff.org failed to provide signature file, but whiffcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "whiff.org and whiffcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Whiff Core binary release signing key installed?"
   fi
